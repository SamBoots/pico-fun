...
{
    st7789_draw_pixel(a_ctx, a_x, a_y, a_color);
    return 1;
}
...
{
    st7789_draw_8x16glyphs(a_ctx, a_str, a_len, a_spacing, a_scale, a_color, a_x, a_y);
    return 1;
}
...
{
    st7789_draw_4x8glyphs(a_ctx, a_str, a_len, a_spacing, a_scale, a_color, a_x, a_y);
    return 1;
}
...
int render_8x16glyphs_2x2border(render_context_t* a_ctx, const char* a_str, uint16_t a_len, uint16_t a_spacing, uint16_t a_scale, uint16_t a_color, uint16_t a_x, uint16_t a_y)
{
    ...
    st7789_draw_8x16glyphs(a_ctx, a_str, a_len, a_spacing, a_scale, a_color, a_x + padding, a_y + padding);
    // now draw border
    
    for (int x = border_x0; x < border_x1; x++)
    {
        st7789_draw_pixel(a_ctx, x, border_y0, a_color);
        st7789_draw_pixel(a_ctx, x, border_y1, a_color);
    }
    for (int y = border_y0; y < border_y1; y++)
    {
        st7789_draw_pixel(a_ctx, border_x0, y, a_color);
        st7789_draw_pixel(a_ctx, border_x1, y, a_color);
    }
...
int render_4x8glyphs_2x2border(render_context_t* a_ctx, const char* a_str, uint16_t a_len, uint16_t a_spacing, uint16_t a_scale, uint16_t a_color, uint16_t a_x, uint16_t a_y)
{
    ...
    st7789_draw_4x8glyphs(a_ctx, a_str, a_len, a_spacing, a_scale, a_color, a_x + padding, a_y + padding);
    // now draw border
...
{
    st7789_draw_rect(a_ctx, a_x, a_y, a_w, a_h, a_color);
    return 1;
}
