...
{
    if (a_scl_len * a_context->pixel_size > sizeof(a_context->row_buf))
    {
        return 0;
    }
    st7789_write16(a_context, a_context->row_buf, a_scl_len);
    return 1;
}
...
int render_fill_area(render_context_t* a_context, uint16_t a_x, uint16_t a_y, uint16_t a_w, uint16_t a_h, uint16_t a_color)
{
    ...
    for (int x = 0; x < a_w; x++) {
        a_context->row_buf[x * 2]     = a_color >> 8;
        a_context->row_buf[x * 2 + 1] = a_color & 0xFF;
    }
